const sw = [
  "枪决女犯",
  "炸药的制作配方",
  "出售M1911",
  "卖钢珠枪QQ",
  "子弹货到付款",
  "高爆炸药的基本配方",
  "普萘洛尔",
  "军用弓弩专卖网",
  "黑火药的配方",
  "TNT炸弹制作",
  "促红细胞生成素",
  "地奈德",
  "枪的制",
  "军品军刺网",
  "你怎么用土办法做武器",
  "下硝化甘油的制作方法",
  "销售电手狗",
  "炸弹制作手册 硫酸",
  "出售国产军用54手枪",
  "防身武器手枪",
  "极端武力直销网",
  "原子弹配方",
  "核弹头的制造",
  "tamoxifen",
  "九评",
  "出售气枪",
  "售单管",
  "苯巴比妥",
  "买卖64狗",
  "fa轮",
  "出售猎枪 电话",
  "如何自制燃烧弹",
  "硝酸甘油炸弹制作方法",
  "售火药",
  "气枪猎枪麻醉枪",
  "炸弹制作从入门",
  "氢弹手工制作方法",
  "枪销售",
  "售纯度",
  "军刀网军刀专卖",
  "大法弟子",
  "弓弩专卖",
  "正见网",
  "自制手枪哪里买  ",
  "三利达弓弩配件",
  "防卫刀具专卖",
  "出售美军现役军刀",
  "仿品军刺网",
  "氯胺酮",
  "小猎人弓弩网",
  "军刀直刀军品网",
  "军用手枪钢珠枪",
  "各类军用枪",
  "弓弩商城",
  "哪里有卖双管猎",
  "军用弓弩公司",
  "赵氏弓弩专卖店",
  "法轮",
  "杜冷丁",
  "军用刀具军品网",
  "供应弓弩麻醉箭",
  "售氯胺",
  "弓弩专卖网",
  "原子弹制作",
  "火药制作简易炸弹",
  "猎枪麻醉枪钢珠枪",
  "西点军品军刀网",
  "制作性能优良的燃烧弹",
  "阳江刀具军品网",
  "猎枪仿真枪QQ",
  "爆破和杀伤燃烧弹",
  "供应弩捕狗箭",
  "军刀网",
  "工字汽狗麻醉枪",
  "盘古",
  "砍刀精品折刀专卖",
  "弓弩专卖网货到付款",
  "售狗子",
  "军用54出售",
  "西布曲明",
  "售热武",
  "炸药配方与制作",
  "弓弩营销中心",
  "钢珠弓弩专卖网",
  "鸡尾酒炸弹制作",
  "三步倒麻醉弩箭销售",
  "气枪/猎枪/钢珠枪",
  "中国弩弓专卖",
  "出售手狗",
  "销售气手狗",
  "k粉",
  "猎枪出售/枪",
  "suicide",
  "出售开山刀军刺",
  "弓弩直销",
  "TNT 炸弹的制作",
  "炸弹制作从入门到精",
  "土炸药成份",
  "军刀军刺甩棍",
  "弓弩网 ",
  "工字牌气枪出售专卖",
  "硝酸甘油炸弹制作",
  "C4炸药",
  "电话定位器",
  "出售炸药QQ",
  "战术军刀出售",
  "benzodiazepines",
  "自制炸药方法",
  "氧化亚铊",
  "三步倒弩箭专卖",
  "供应麻醉箭三步倒",
  "原子弹方法",
  "原子弹制作方法",
  "逢九必乱",
  "售枪购买手枪",
  "电话交友",
  "出售手枪",
  "大麻",
  "狩猎弓弩麻醉箭",
  "国产汽狗专卖|气枪货",
  "亚砷酸钠",
  "伪火",
  "DIY核弹",
  "出售步枪",
  "北方先锋军刀",
  "出售54式手枪",
  "猎枪|真枪",
  "电击枪",
  "手工猎刀户外刀具",
  "枪子弹",
  "弓弩狩猎网",
  "折叠狗QQ",
  "诸世纪",
  "三利达弓弩军刀",
  "阿兰德龙野营刀",
  "硝酸甘油制作",
  "全金属仿真枪专卖",
  "制作硝化甘油",
  "勃朗宁军刀",
  "弓弩销售",
  "军刀甩棍销售",
  "出售猎狗",
  "购买自爆材料",
  "燃烧剂的简易制备",
  "推背图",
  "出售军刀",
  "三步倒麻醉箭专卖",
  "直刀匕首直销网",
  "电话窃听",
  "氧化二丁基锡",
  "麻古",
  "海luo因",
  "朱瑟里诺",
  "DIY原子弹",
  "诺查丹玛斯",
  "李宏志",
  "亚硒酸氢钠",
  "气枪",
  "猎枪哪里有卖",
  "气枪价格 仿真气枪",
  "打狗弓弩三步倒",
  "户外军刀网",
  "安眠酮",
  "地西泮",
  "气枪迷魂药",
  "黑火药配方比例",
  "军用77出售",
  "锡峰气枪出售",
  "mdma",
  "氧氯化磷",
  "售枪支",
  "军刀专 卖",
  "阳江刀具批发网",
  "出售雷管炸药",
  "防卫电棍出售",
  "售健卫",
  "军刀价格军用刀具",
  "制作火药配方",
  "高压气枪专卖店",
  "哪里有钢珠枪",
  "军刀匕首直销网",
  "出售气狗",
  "退dang",
  "简易燃烧弹",
  "汽油三分之一白糖混合",
  "供应精品弓弩",
  "哪里有气枪卖",
  "弓弩免定金货到付款",
  "小型弓弩专卖店",
  "冷钢刀具军品网",
  "硝酸铵的简便制取方法",
  "出售进口金属枪模",
  "买卖警察枪支",
  "中国弓驽网",
  "促性腺激素",
  "最新销售汽狗",
  "售弹簧刀",
  "买卖军用枪支",
  "自sha",
  "三步倒捕狗药",
  "strychnine",
  "falungong",
  "军刀专卖军刺",
  "出售气枪 电话",
  "原装弹",
  "炸药的成分",
  "法一轮一功",
  "济世灵文",
  "手教你做原子弹",
  "法x功",
  "有军用枪支出售",
  "亚硒酸镁",
  "军刀军品网",
  "防身手枪QQ",
  "自fen",
  "里有汽枪出售",
  "tuidang",
  "提供气狗专卖店",
  "法o功",
  "塑胶炸弹",
  "各式气枪出售",
  "弓弩论坛 ",
  "猎枪子弹底火专卖",
  "售手枪QQ",
  "爱他死",
  "燃烧瓶制作",
  "赵氏弓弩销售",
  "售猎枪",
  "9ping",
  "气枪货到付款汽狗出售",
  "冰毒",
  "逢八必灾",
  "折刀砍刀军品网",
  "弓弩专卖店",
  "观音法门",
  "野营砍刀户外军刀",
  "出售手枪 QQ",
  "自制手枪哪里有卖",
  "海洛因",
  "出售:高仿真枪",
  "炸弹制作",
  "水浴法制TNT",
  "炸药硝酸甘油制法",
  "铅弹 上海工字气枪",
  "炸药及其制造方法",
  "军用弓弩批发",
  "简易核弹头的制造",
  "麻醉枪猎枪军用",
  "明慧网",
  "泼尼松",
  "阳江军品军刀网",
  "原子弹制造完全手册",
  "出售猎枪 QQ",
  "zi杀",
  "亚砷酸钾",
  "三利达弓弩专卖网",
  "供应军用手枪",
  "亚硒酸二钠",
  "弓弩专卖店网",
  "秃鹰气枪货到付款",
  "仿品军刺出售",
  "气枪购买|气枪组装",
  "军刀直刀专卖",
  "亚硒酸",
  "售左轮",
  "黑索金的制造过程",
  "逢8必灾",
  "Zha药制造进程",
  "枪模",
  "娥眉气枪出售",
  "逢9必乱",
  "三步倒药箭批发",
  "买卖54狗",
  "集束炸弹制作",
  "燃烧弹的制作方 ",
  "出售外国枪模",
  "高压气枪气枪子弹",
  "简易炸药",
  "著名精品折刀出售",
  "售虎头",
  "售步枪",
  "怎么用土办法做武器",
  "户外军刀出售",
  "液体炸药配方",
  "亚硝酰乙氧",
  "统一教",
  "火药配方",
  "自制手弩",
  "炸药的制作",
  "androst",
  "汽狗高压汽枪",
  "极端武力军品网",
  "供应麻醉箭批发",
  "志洪李",
  "莫达非尼",
  "黑火药配方",
  "出售军刺",
  "弓弩亲兄弟货到付款",
  "明慧周报",
  "黑火药的制作",
  "胰岛素样生长因子",
  "转让 猎枪",
  "售麻醉",
  "极端武力折刀",
  "中国弓弩狩猎网",
  "亚硒酸钠",
  "弓弩商城专卖",
  "美沙酮",
  "九ping",
  "卖64手枪",
  "警用刀具出售",
  "人类灭亡进程表",
  "弓弩专营",
  "可塑炸弹制作方法",
  "防卫棍刀户外刀具",
  "代血浆",
  "出售警用手铐|仿真枪",
  "火药原理",
  "赵氏弓弩专卖网",
  "供应汽枪",
  "供应军用弓弩专卖",
  "地塞米松",
  "9评",
  "出售手枪 电话",
  "提供专业及时的气狗",
  "手柄制作炸弹引爆器",
  "野营刀具军品网",
  "阻击枪/汽枪/高压气枪",
  "弹药制造方法",
  "自制炸药配方",
  "手工猎刀专卖",
  "如何制硝化甘油",
  "燃烧剂的简易制作",
  "野营刀专卖",
  "testosterone",
  "推bei图",
  "heroin",
  "出售炸药",
  "炸药出售 ",
  "麻醉弓弩专卖店",
  "高性能爆炸物质混合",
  "简易导弹制作",
  "手把手教你制作原子弹",
  "狩猎弓弩专卖网",
  "雄烯二醇",
  "黑火药的成分",
  "手把手教你做炸弹",
  "买卖军狗",
  "开山刀砍刀出售",
  "销售运动步枪",
  "狩猎弓弩专卖",
  "三分钟教你原子弹",
  "亚硝酸乙酯",
  "起爆器",
  "凯他敏",
  "气枪仿真枪出售",
  "原子弹 制作方法",
  "集体自杀",
  "枪出售",
  "电话监",
  "「红烧兔子」大餐",
  "titor",
  "手教你制作原子弹",
  "西点军刀网",
  "根达亚文明",
  "制作炸弹首先要",
  "弓弩麻醉箭",
  "售军用",
  "法lg",
  "尼可刹米",
  "手槍买卖自制手枪",
  "电狗",
  "售子弹",
  "车仑工力",
  "哪里能买到自制手枪  ",
  "供应麻醉箭",
  "供应钢珠弓弩",
  "写的炸弹制作教程",
  "美国军刀出售",
  "麻醉枪出售可验货",
  "原子弹清单",
  "刀具直销网",
  "弓弩专卖货到付款",
  "硝酸甘油",
  "出售气枪 QQ",
  "核弹制作方法",
  "黑火药和硝酸钾农药混合",
  "燃烧瓶的制作方法",
  "期出售手枪气枪猎",
  "苯丙胺",
  "狩猎器材弓弩",
  "钢珠枪小口径步枪",
  "安非他命",
  "炸药",
  "呋塞米",
  "防卫著名军刀出售",
  "售手枪",
  "三箭气枪出售",
  "哪里有连发猎枪买卖",
  "中国战神军用弓弩",
  "弓弩麻醉镖",
  "炸弹制作入门",
  "用火药混合塑料",
  "简易炸弹",
  "出售火枪 –游戏",
  "C4塑胶炸药",
  "法lun",
  "出售猎枪",
  "【手枪出售】联系电话",
  "怎么自制燃烧弹",
  "92式手枪出售",
  "新唐人",
  "炸弹配方",
  "长期出 售手枪",
  "电鸡",
  "折刀砍刀专卖",
  "仙鹤气枪出售",
  "原子弹制作 方法",
  "阿兰得龙野营刀具网",
  "大纪元",
  "哪里有真枪卖买",
  "各种炸药配方大全",
  "专业弓弩网",
  "军刀专卖刀具批发",
  "三去车仑",
  "基本配方是硝酸甘",
  "折刀专卖网",
  "自动跳刀专卖",
  "售雷管",
  "咖啡因",
  "莫洛托夫鸡尾酒的配方",
  "出售各类猎狗",
  "防卫刀具军品网",
  "手枪式折叠三用弩",
  "中国教徒",
  "燃烧弹 制作",
  "三棱军刺专卖",
  "中国弓弩直销",
  "枪手",
  "供应军用弩折叠弩",
  "电警棒",
  "cocain",
  "轻型巡航导弹的制作",
  "出售袖珍手枪",
  "甲基安非他明",
  "冷钢刀具直销网",
  "电话拦截器",
  "气枪专卖网",
  "李洪志",
  "藏字石",
  "进口气枪气枪子弹",
  "郑州弓弩专卖",
  "南方军刀网",
  "购买枪支QQ",
  "售五四",
  "军刀直销网",
  "热武器QQ",
  "出售狙击枪",
  "防卫甩棍出售",
  "狩猎器材弓弩专卖",
  "按照马雅历法",
  "出售手枪=货到付款",
  "硝铵炸药配方",
  "三步倒麻醉箭",
  "tnt炸药成分",
  "枪械制",
  "赵氏弓弩专卖",
  "小口径秃鹰沙漠",
  "高爆炸药基本配方",
  "哪里有枪出售",
  "燃烧弹",
  "出售跳刀",
  "司马仿真枪模型QQ",
  "制作简易弓弩 ",
  "tnt炸药配方",
  "硝酸甘油炸药制作",
  "供应弩用麻醉箭",
  "军用弓弩专卖店",
  "简易原子弹教程",
  "野营刀具专卖",
  "diamorphine",
  "正品军刺出售",
  "三唑仑",
  "集团出售手枪",
  "卖92手枪",
  "刺刀专卖网",
  "冷钢刀具专卖",
  "兴奋剂",
  "出售麻醉枪免收任何定金",
  "军刀电棍销售",
  "猎枪销售手枪销售气枪",
  "C4炸药的成分",
  "气枪联系QQ",
  "轮子功",
  "手枪气* 枪猎",
  "供应秦氏弓弩",
  "fl功",
  "标准炸弹教程",
  "高压狗QQ",
  "钢珠弓弩专卖店",
  "出售三棱刀",
  "售三棱",
  "弓弩网",
  "军刺野营砍刀出售",
  "出售炸药 电话",
  "64式手枪出售",
  "燃烧弹的制作方法",
  "高压气枪|气枪货到付款",
  "cannabis",
  "吗啡",
  "zz炸弹的制作方法",
  "中国弓弩专卖网",
  "汽车炸弹制作",
  "组装手枪  ",
  "防卫刀具直销网",
  "猎枪专卖店",
  "简易大威力炸弹",
  "弓弩特许经销",
  "弓弩制作",
  "自制燃烧弹",
  "阳江刀具专卖",
  "甲睾酮",
  "人体炸弹制作流程",
  "哪能买到真枪",
  "阳江刀具直销网",
  "亚砷（酸）酐",
  "销售小口径步枪",
  "燃烧弹的制作方",
  "尼泊尔军刀出售",
  "弓弩一百专卖店",
  "中国户外刀具网",
  "戈博军刀",
  "供应弓弩",
  "adrenaline",
  "弓弩有限公司",
  "售一元硬",
  "教你怎么用土办法做武器",
  "猎枪手枪买卖",
  "阿兰德龙户外",
  "退党",
  "淘宝气枪货到付款",
  "flg",
  "供应三步倒麻醉箭",
  "弓弩图纸",
  "出售军刺军刀",
  "炸药的配方",
  "发论工",
  "氧化汞",
  "莫洛托夫燃烧弹",
  "气枪价格，气枪专卖店",
  "在家里做原子弹",
  "手枪|阻击枪",
  "出售军品军刀",
  "可塑炸弹制作",
  "枪货到",
  "原子弹DIY教程",
  "氵去车仑",
  "追风弓弩麻醉箭专卖",
  "氧化铊",
  "超越红墙",
  "哪里有真枪出售",
  "中国弓弩网",
  "教你制作原子弹",
  "出售仿真枪",
  "真善忍",
  "dajiyuan",
  "电话追杀系统",
  "匕首蝴蝶甩刀专卖",
  "麻黄草",
  "ketamine",
  "仿真枪QQ",
  "防卫野营砍刀出售",
  "法0功",
  "出售弹簧刀",
  "轮功",
  "集团出售手枪气枪",
  "erythropoietin",
  "供应三利达弓弩麻醉箭",
  "炸药制造生产工艺",
  "气狗出售 进口气狗",
  "野营开山刀军刺",
  "售防身",
  "防卫棍刀出售",
  "气枪/出售/专卖网",
  "原子弹制作方法DIY",
  "售假币",
  "高压气枪|电狗专卖",
  "jiuping",
  "气枪看货加QQ",
  "原子弹制作手册",
  "燃烧弹制作",
  "恐怖分子自己制造炸弹",
  "销售/专卖/买卖77式手枪",
  "出售汽枪",
  "三利达弓弩直营",
  "新型毒品",
  "弓弩批发",
  "火药的配方",
  "野营军刀出售",
  "酸甘油炸药",
  "清海无上师",
  "雷管出售",
  "轮法功",
  "diacetylmorphine",
  "军刀图片砍刀",
  "氯噻嗪",
  "恐怖分子傻瓜手册",
  "气狗 高压气枪",
  "鸦片",
  "ATOM弹制造",
  "黑火药比例",
  "阿芙蓉",
  "各类军用枪支",
  "中国复兴党",
  "连锁加盟",
  "毒龙",
  "销售热线",
  "阿波罗网",
  "客服",
  "代理",
  "在线播放",
  "兼职",
  "买小车",
  "本店",
  "信用卡提现",
  "六位qq",
  "口交",
  "网络工作",
  "网购",
  "BT",
  "狼友",
  "加盟连锁",
  "售肾",
  "到货",
  "原音铃声",
  "腾讯客服电话",
  "性伴侣",
  "扣扣",
  "全职",
  "操逼",
  "楼凤",
  "周小川",
  "下载速度",
  "妓女",
  "胸推",
  "婴儿汤",
  "找个爸爸",
  "有意者",
  "考中答案",
  "刘明康",
  "小姐",
  "答an",
  "帮忙点下",
  "用刀横向切腹",
  "救市",
  "质押贷款",
  "婊子",
  "咪咪",
  "资金周转",
  "小额贷款",
  "六合彩",
  "淘宝",
  "招聘",
  "套牌车",
  "请点击进入",
  "足交",
  "代开发票",
  "代孕妈妈",
  "兼值",
  "JS",
  "吞精",
  "详情请进入",
  "替考试",
  "尚福林",
  "代生孩子",
  "位的qq",
  "足球投注",
  "乳方",
  "完全自杀手册",
  "口暴",
  "免费订购热线",
  "款到发货",
  "帮忙点一下",
  "广告代理",
  "出售答案",
  "回复可见",
  "刻章办",
  "无抵押贷款",
  "私家侦探",
  "推油",
  "免费索取",
  "txt下载",
  "股市圈钱",
  "曾道人",
  "崩盘",
  "6位qq",
  "资金短缺",
  "QQ",
  "3P",
  "蚁力神",
  "证监会",
  "桑拿",
  "da案",
  "免费使用",
  "免费二级域名",
  "电脑传讯",
  "网络",
  "针孔摄象",
  "四海帮",
  "代购",
  "技师",
  "SM",
  "微店",
  "找个妈妈",
  "包夜",
  "高清在线",
  "全套",
  "孔丹",
  "借腹生子",
  "调查婚外情",
  "LY",
  "玛雅网",
  "视频来源",
  "个qb",
  "专业代理",
  "口爆",
  "私人侦探",
  "送qb",
  "低价出售",
  "全集在线",
  "隐形耳机",
  "地下钱庄",
  "改卷内幕",
  "dpp大法",
  "普萘洛尔",
  "促红细胞生成素",
  "地奈德",
  "天怒",
  "KC嘉年华",
  "脱裤门",
  "tamoxifen",
  "炼功",
  "八级地震毫无预报",
  "阀沦",
  "考联盟",
  "苯巴比妥",
  "法论",
  "各种发票",
  "武侯祠",
  "fa轮",
  "买小车",
  "联总之声传单",
  "轮大",
  "杨佳",
  "yang佳",
  "五不争鸣论坛",
  "纠纷",
  "地震来得更猛烈",
  "踩踏事故",
  "无界浏览器",
  "大法弟子",
  "易达网络卡",
  "原音铃声",
  "腾讯客服电话",
  "筏伦",
  "氯胺酮",
  "无网界浏览",
  "大麻油",
  "morphine",
  "垡纶",
  "提前预测",
  "无帮国",
  "洗脑",
  "钓鱼台",
  "李四光预测",
  "法轮",
  "珐纶",
  "杜冷丁",
  "懊运",
  "旦科",
  "答an",
  "透视眼镜",
  "砝仑",
  "万里大造林",
  "五套功法",
  "资金周转",
  "喂奶门",
  "传九促三",
  "乏伦",
  "酒瓶门",
  "h1n1",
  "法轮功",
  "套牌车",
  "请点击进入",
  "KC提示",
  "护法",
  "打人",
  "尚福林",
  "考考邓",
  "考试保",
  "位的qq",
  "西布曲明",
  "大法",
  "款到发货",
  "功友",
  "考后付款",
  "考试包过",
  "帮忙点一下",
  "高官子女",
  "反奥",
  "真、善、忍",
  "Falundafa",
  "回复可见",
  "贺立旗",
  "短信广告",
  "私家侦探",
  "抡功",
  "联总这声传单",
  "k粉",
  "当官靠后台",
  "发囵",
  "珐囵",
  "李树菲",
  "打砸抢",
  "白皮书",
  "乏论",
  "fa lun",
  "垡囵",
  "瓮安事件",
  "乏仑",
  "加了服",
  "手机复制",
  "砝纶",
  "精神药品",
  "法伦",
  "法沦",
  "免费使用",
  "罚伦",
  "benzodiazepines",
  "迷药",
  "清仓",
  "川b26931",
  "高官",
  "代讨债",
  "透视功能",
  "阀仑",
  "代办学",
  "大麻",
  "考前密卷",
  "法lun功",
  "盗窃",
  "孔丹",
  "筏仑",
  "网关",
  "恭喜你的号码",
  "伐论",
  "乏抡",
  "低价出售",
  "全集在线",
  "代孕",
  "移民",
  "教徒",
  "中国复兴党",
  "短信群发器",
  "杨j",
  "鸿志",
  "麻古",
  "聂树斌",
  "阿波罗网",
  "海luo因",
  "黑车",
  "垡抡",
  "安眠酮",
  "地西泮",
  "真善美",
  "讲法",
  "打死经过",
  "mdma",
  "奥晕",
  "车仑",
  "邝锦文",
  "透视器",
  "信用卡提现",
  "阀纶",
  "narcotic",
  "高干子女",
  "法L功",
  "砝囵",
  "罢课",
  "伐沦",
  "考试作弊器",
  "售肾",
  "法*功",
  "短信群发",
  "阀论",
  "筏纶",
  "茳澤民",
  "张丹红",
  "闳志",
  "发沦",
  "垡仑",
  "促性腺激素",
  "下载速度",
  "古兰经",
  "strychnine",
  "车库",
  "考研考中",
  "考中答案",
  "代理票据",
  "传单",
  "质押贷款",
  "砝伦",
  "小额贷款",
  "阀抡",
  "六合彩",
  "发伦",
  "珐沦",
  "yangjia",
  "考试答案",
  "透视眼睛",
  "纽崔莱七折",
  "代开发票",
  "爱他死",
  "黑手党",
  "冰毒",
  "氵去",
  "高利贷",
  "海洛因",
  "龙小霞",
  "零八奥运艰",
  "氵去车仑工力",
  "洪吟",
  "足球投注",
  "监听王",
  "法谪",
  "麻醉药",
  "完全自杀手册",
  "免费订购热线",
  "隐瞒地震",
  "红志",
  "筏抡",
  "虹志",
  "泼尼松",
  "刻章办",
  "宏法",
  "考试枪",
  "小6灵通",
  "发抡功",
  "代办发票",
  "航空母舰",
  "乏纶",
  "梦网洪志",
  "免费索取",
  "功法",
  "砝抡",
  "奥你妈的运",
  "考试机构",
  "曾道人",
  "崩盘",
  "航空售票",
  "罚抡",
  "珐抡",
  "奥孕",
  "法十轮十功",
  "客户端非法字符",
  "da案",
  "高薪养廉",
  "KC短信",
  "UP8新势力",
  "翁安",
  "发纶",
  "拆迁",
  "四海帮",
  "黑庄",
  "转法轮",
  "瘟疫爆发",
  "家le福",
  "卡号",
  "经文",
  "75事件",
  "KC网站",
  "佳静安定片",
  "罚纶",
  "法抡功",
  "五出三进",
  "androst",
  "震其国土",
  "法西斯",
  "莫达非尼",
  "乏沦",
  "个qb",
  "胰岛素样生长因子",
  "px项目",
  "伐抡",
  "伦功",
  "美沙酮",
  "摇头丸",
  "罚沦",
  "洪哲胜",
  "UP新势力",
  "连锁加盟",
  "基督教",
  "代血浆",
  "罚仑",
  "法轮佛法",
  "洪志",
  "销售热线",
  "地塞米松",
  "垡论",
  "砝论",
  "砝轮",
  "在线播放",
  "地下教会",
  "透视扑",
  "伐仑",
  "代写论文",
  "testosterone",
  "heroin",
  "天葬",
  "伐囵",
  "雄烯二醇",
  "乖乖粉",
  "六位qq",
  "考试作弊",
  "发论功",
  "安魂网",
  "杀警",
  "发伦功",
  "乏囵",
  "凯他敏",
  "大师",
  "中印边界谈判结果",
  "珐仑",
  "宏志",
  "杨树宽",
  "刻章",
  "代考",
  "尼可刹米",
  "找个爸爸",
  "代開",
  "摩门教",
  "真理教",
  "如6意通",
  "王奉友",
  "帮忙点下",
  "无界浏览",
  "救市",
  "弟子",
  "发抡",
  "大麻树脂",
  "苯丙胺",
  "代孕妈妈",
  "法 轮 功",
  "安非他命",
  "wengan",
  "段录定",
  "伐伦",
  "呋塞米",
  "详情请进入",
  "替考试",
  "针刺",
  "weng安",
  "骂四川",
  "falun",
  "蟾蜍迁徙",
  "新疆骚乱",
  "打针",
  "Flg",
  "透视镜",
  "代追债",
  "法院",
  "达赖",
  "法囵",
  "弘志",
  "代办文",
  "这不是天灾",
  "代药物毒品类：血浆",
  "珐论",
  "砝沦",
  "txt下载",
  "筏论",
  "洪传",
  "6位qq",
  "资金短缺",
  "发论公",
  "筏轮",
  "短信商务广告",
  "蚁力神",
  "打砸办公",
  "咖啡因",
  "灵动卡",
  "珐伦",
  "饭菜涨价",
  "法轮大法",
  "周正毅",
  "针孔摄象",
  "手机铃声",
  "考前付",
  "答案卫星接收机",
  "伐轮",
  "透视仪",
  "找个妈妈",
  "达赖喇嘛",
  "考前答案",
  "cocain",
  "甲基安非他明",
  "借腹生子",
  "调查婚外情",
  "玛雅网",
  "私人侦探",
  "送qb",
  "考机构",
  "法功",
  "回教",
  "陈相贵",
  "钓鱼岛",
  "贯通两极法",
  "摸nai门",
  "泓志",
  "发正念",
  "代写毕",
  "falu",
  "diamorphine",
  "发愣",
  "三唑仑",
  "法正",
  "高官互调",
  "化工厂爆炸",
  "筏囵",
  "y佳",
  "罚囵",
  "袭警",
  "代办各",
  "发瞟",
  "兴奋剂",
  "无界",
  "喝一送一",
  "乌鲁木齐",
  "各个银行全称",
  "五不",
  "法抡",
  "惨奥",
  "车臣",
  "家l福",
  "珐轮",
  "阀囵",
  "加盟连锁",
  "cannabis",
  "屙民",
  "吗啡",
  "法仑",
  "无网界",
  "yuce",
  "全1球通",
  "罢工",
  "考试答案",
  "婴儿汤",
  "法纶",
  "代办制",
  "考前预测",
  "垡伦",
  "甲睾酮",
  "刘明康",
  "考前发放",
  "法谪功",
  "汉芯造假",
  "用刀横向切腹",
  "发论",
  "乏轮",
  "628事件",
  "讨回工资",
  "adrenaline",
  "阀伦",
  "诚信通手机商城",
  "联总",
  "闵维方",
  "恭喜您的号码",
  "神通加持法",
  "麦当劳被砸",
  "食堂涨价",
  "代生孩子",
  "成本价",
  "党校安插亲信",
  "复制",
  "懊孕",
  "法愣",
  "豆腐渣",
  "法.轮.功",
  "广告代理",
  "号码百事通",
  "考试联盟",
  "发轮",
  "珐.輪功",
  "真善忍",
  "出售答案",
  "凹晕",
  "打死人",
  "麻黄草",
  "无抵押贷款",
  "ketamine",
  "轮功",
  "erythropoietin",
  "发仑",
  "筏沦",
  "股市圈钱",
  "光祖",
  "代辦",
  "透视药",
  "法輪",
  "联4通",
  "高干子弟",
  "考试作弊器",
  "证监会",
  "代您考",
  "基督",
  "高息贷款",
  "垡轮",
  "免费二级域名",
  "电脑传讯",
  "天皇",
  "尹方明",
  "代表烦",
  "法会",
  "下法轮",
  "新型毒品",
  "垡沦",
  "高清在线",
  "联通贵宾卡",
  "荭志",
  "观世音",
  "代理发票",
  "diacetylmorphine",
  "氯噻嗪",
  "视频来源",
  "三水法轮",
  "专业代理",
  "鸦片",
  "改卷内幕",
  "隐形耳机",
  "阿芙蓉",
  "地下钱庄",
  "周小川",
  "地震预测",
  "欲女",
  "淫虐",
  "小xue",
  "美穴",
  "推油",
  "原味内衣",
  "肉棒",
  "亚情",
  "草你吗",
  "处男",
  "乳爆",
  "嫩女",
  "骚浪",
  "风月大陆",
  "抽一插",
  "美幼",
  "阴户",
  "迷昏口",
  "艳情小说",
  "颜射",
  "g片",
  "调教",
  "妈了个逼",
  "沙比",
  "精子",
  "性虐",
  "相奸",
  "阴部",
  "sm",
  "a4y",
  "炮友",
  "操我",
  "乱交",
  "日逼",
  "女优",
  "裸照",
  "三级片",
  "gay片",
  "插阴",
  "抽插",
  "食精",
  "口淫",
  "嫩逼",
  "强暴",
  "喷精",
  "舔阴",
  "兽欲",
  "体奸",
  "美腿",
  "乳沟",
  "性饥渴",
  "骚水",
  "amateur",
  "发生关系",
  "妓女",
  "口交",
  "肏你",
  "色逼",
  "被操",
  "卧槽",
  "美逼",
  "性爱",
  "肉穴",
  "巨乳",
  "操烂",
  "鸡吧",
  "浪妇",
  "淫情",
  "淫水",
  "性欲",
  "淫声浪语",
  "干穴",
  "色情表演",
  "插进",
  "迷魂藥",
  "露b",
  "插你",
  "狗日的",
  "morphine",
  "荡女",
  "阴蒂",
  "淫术炼金士",
  "开苞",
  "中年美妇",
  "仓井空",
  "淫教师",
  "张筱雨",
  "迷幻藥",
  "淫肉",
  "被干",
  "性交",
  "a片",
  "密穴",
  "性感诱惑",
  "裸陪",
  "干你妈",
  "傻比",
  "色妹妹",
  "麻痹的",
  "狼友",
  "逼奸",
  "操你娘",
  "淫母",
  "成人电影",
  "放尿",
  "a片",
  "肉茎",
  "按摩棒",
  "淫威",
  "tokyohot",
  "全家不得好死",
  "熟女",
  "少年阿宾",
  "肉唇",
  "媚外",
  "玉穴",
  "成人视",
  "乳交",
  "成人网站",
  "迷奸药",
  "舔脚",
  "舔脚",
  "不穿",
  "死逼",
  "裸体写真",
  "凌辱",
  "淫河",
  "射颜",
  "套弄",
  "性息",
  "调教",
  "真他妈",
  "xing伴侣",
  "狂操",
  "屌",
  "阉割",
  "美乳",
  "色诱",
  "淫妻",
  "被操",
  "爆草",
  "菊穴",
  "骚比",
  "轮暴",
  "推油",
  "肉逼",
  "轮操",
  "操他妈",
  "体位",
  "春药",
  "奶子",
  "成人游戏",
  "淫妇",
  "人妻",
  "舞女",
  "淫照",
  "成人文学",
  "精神药品",
  "性饥渴",
  "潮喷",
  "穴口",
  "好嫩",
  "阴唇",
  "色诱",
  "性技巧",
  "荡女",
  "爽片",
  "下贱",
  "淫书",
  "贱货",
  "抽插",
  "吃精",
  "干死你",
  "黑逼",
  "按摩",
  "迷药",
  "色b",
  "马勒",
  "砲友",
  "贱b",
  "吃精",
  "你妈的",
  "小穴",
  "乱奸",
  "插比",
  "轮操",
  "乳爆",
  "淫教师",
  "写真",
  "美女裸体",
  "sm女王",
  "日烂",
  "酥痒",
  "肥逼",
  "操你妈",
  "煞笔",
  "成人文学",
  "精液",
  "成人卡通",
  "h动画",
  "骚水",
  "汤加丽",
  "成人dv",
  "爆草",
  "男公关",
  "谜奸药",
  "陰唇",
  "买春",
  "夜勤病栋",
  "蜜穴",
  "做爱",
  "淫糜",
  "浪叫",
  "作爱",
  "yin荡",
  "欠干",
  "放尿",
  "双臀",
  "聊性",
  "全家死光",
  "淫液",
  "射颜",
  "鸡奸",
  "兽交",
  "爆你菊",
  "暴奸",
  "聊性",
  "抓胸",
  "漏乳",
  "煞逼",
  "换妻俱乐部",
  "暴乳",
  "成人电",
  "摸奶",
  "狗草",
  "肉洞",
  "性福情",
  "淫兽",
  "美乳",
  "淫液",
  "松岛枫",
  "狗杂种",
  "兼职",
  "寂寞女",
  "激情",
  "性技巧",
  "龟头",
  "情趣用品",
  "乳沟",
  "迷魂香",
  "narcotic",
  "淫贱",
  "爆乳",
  "拔出来",
  "无耻",
  "小穴",
  "阴道",
  "一ye情",
  "乱伦小",
  "校鸡",
  "淫魔",
  "迷情水",
  "粉穴",
  "狼友",
  "巨乳",
  "爱女人",
  "你吗b",
  "插逼",
  "阴核",
  "傻逼",
  "强暴",
  "寂寞男",
  "强奸",
  "骚女",
  "射爽",
  "美少妇",
  "淫书",
  "肉茎",
  "狂插",
  "性交",
  "大力抽送",
  "干死你",
  "g点",
  "日逼",
  "舞女",
  "夏川纯",
  "阴b",
  "淫荡自慰器",
  "刺激",
  "赤裸",
  "酥痒",
  "仓井空",
  "玉乳",
  "蜜穴",
  "兽交",
  "拳交",
  "校鸡",
  "前凸后翘",
  "色色",
  "熟妇",
  "性息",
  "亚情",
  "性交图片",
  "阴间来电",
  "暴乳",
  "夏川纯",
  "淫亵",
  "美幼",
  "狗娘养",
  "性感妖娆",
  "发情",
  "暴奸",
  "肉棍",
  "骚妇",
  "奈美",
  "学生妹",
  "一夜欢",
  "迷奸",
  "色情网站",
  "裸聊网",
  "一本道",
  "颜射",
  "麻醉药",
  "淫电影",
  "爱液",
  "肉缝",
  "骚穴",
  "大乳",
  "舔阴",
  "小xue",
  "情色",
  "迷情药",
  "秘唇",
  "虎骑",
  "擦你妈",
  "色电影",
  "色界",
  "adult",
  "淫浪",
  "一本道",
  "盗撮",
  "美艳少妇",
  "失身粉",
  "释欲",
  "狗操",
  "诱奸",
  "肏你",
  "h动漫",
  "肥逼",
  "火辣",
  "援交",
  "美腿",
  "美逼",
  "脚交",
  "色逼",
  "anal",
  "内射",
  "肛门",
  "淫亵",
  "铃声",
  "美女写真",
  "自慰",
  "抓胸",
  "淫糜",
  "肏死",
  "cao你",
  "阴道",
  "操你祖宗",
  "体位",
  "菊门",
  "奸情",
  "幼女",
  "肛门",
  "射精",
  "群交",
  "性欲",
  "淫色",
  "乱交",
  "色小说",
  "肉棒",
  "双臀",
  "淫荡",
  "阴茎增大",
  "杨思敏",
  "porn",
  "淫水",
  "阴精",
  "按摩棒",
  "淫声浪语",
  "鸡吧",
  "插暴",
  "肉具",
  "性奴",
  "淫女",
  "阴核",
  "成人色情",
  "大波",
  "黑逼",
  "金麟岂是池中物",
  "几吧",
  "拳交",
  "干死",
  "色情网站",
  "脱内裤",
  "艹你",
  "色欲",
  "少修正",
  "骚嘴",
  "插阴",
  "色情片",
  "一夜情",
  "阴茎",
  "阴唇",
  "色猫",
  "脚交",
  "露b",
  "色情电影",
  "脱光",
  "无码",
  "操我",
  "操黑",
  "裸舞视",
  "菊穴",
  "释欲",
  "摇头丸",
  "应召",
  "豪乳",
  "裹本",
  "成人聊",
  "操逼",
  "后穴",
  "丝袜",
  "风骚",
  "惹火身材",
  "前凸后翘",
  "快感",
  "fuck",
  "厕奴",
  "淫照",
  "你麻痹",
  "口爆",
  "嫩穴",
  "淫娃",
  "吸精",
  "后庭",
  "熟女",
  "18禁",
  "淫荡美女",
  "乳头",
  "迷藥",
  "裤袜",
  "熟妇",
  "封面女郎",
  "杀b",
  "操死",
  "伦理毛",
  "婊子养的",
  "集体淫",
  "妹按摩",
  "g点",
  "荡妇",
  "迷昏藥",
  "轮暴",
  "自拍",
  "乖乖粉",
  "铃木麻",
  "杨思敏",
  "混蛋",
  "喷精",
  "浪逼",
  "发浪",
  "淫荡照片",
  "淫媚",
  "乳房",
  "sb",
  "陰戶",
  "淫叫",
  "丝诱",
  "浪女",
  "迷情粉",
  "色情影片",
  "色猫",
  "操你全家",
  "干你娘",
  "裸聊",
  "成人小说",
  "操你大爷",
  "强奸处女",
  "一夜欢",
  "被干",
  "骚逼",
  "菊门",
  "叫床",
  "奈美",
  "熟母",
  "口交",
  "蜜液",
  "一ye情",
  "巨屌",
  "秘唇",
  "叫床",
  "鸡奸",
  "穴图",
  "春药",
  "母奸",
  "口淫",
  "淫样",
  "阴茎助勃",
  "凌辱",
  "美少妇",
  "花花公子",
  "肉穴",
  "套弄",
  "性交视频",
  "吸精",
  "嫩女",
  "艳情小说",
  "后穴",
  "伦理大",
  "射爽",
  "盗撮",
  "乱伦",
  "性感诱惑",
  "淫兽学园",
  "摸胸",
  "色盟",
  "穴图",
  "摸奶",
  "兽欲",
  "肉欲",
  "骚货",
  "被插",
  "阴部",
  "就去日",
  "淫母",
  "插暴",
  "多人轮",
  "人渣",
  "淫靡",
  "招妓",
  "性感妖娆",
  "插我",
  "一丝不挂",
  "别他吗",
  "鸡巴",
  "穴口",
  "我干",
  "招鸡",
  "无修正",
  "妹上门",
  "包二奶",
  "淫兽学",
  "少年阿宾",
  "奶子",
  "限量",
  "乳交",
  "国产av",
  "好嫩",
  "淫媚",
  "刹笔",
  "粉穴",
  "成人论坛",
  "潮吹",
  "淫魔舞",
  "摸胸",
  "99bb",
  "扌由插",
  "幼交",
  "a4u",
  "插比",
  "淫术炼金士",
  "几吧",
  "xiao77",
  "贱人",
  "裙中性运动",
  "secom",
  "情趣用品",
  "作爱",
  "迷奸粉",
  "h动画",
  "淫兽学园",
  "猛男",
  "色区",
  "草你丫",
  "死全家",
  "文做",
  "你他妈",
  "淫浪",
  "人妻",
  "淫虫",
  "漏乳",
  "我日你",
  "厕奴",
  "自慰",
  "呻吟",
  "荡妇",
  "全裸",
  "全家死绝",
  "人兽",
  "美女上门",
  "幼交",
  "手淫",
  "色情图片",
  "肉具",
  "插b",
  "被插",
  "插b",
  "援助交际",
  "口射",
  "淫叫",
  "口爆",
  "豪乳",
  "成人片",
  "暴淫",
  "肛交",
  "强奸处女",
  "性感少",
  "爆乳",
  "阴茎",
  "少妇",
  "成人电影",
  "文做",
  "粉嫩",
  "屁眼",
  "开苞",
  "插逼",
  "暴淫",
  "强jian",
  "死逼",
  "性虎",
  "揉乳",
  "傻b",
  "口射",
  "阴户",
  "女优",
  "浪女",
  "淫荡视频",
  "内射",
  "强jian",
  "妈逼",
  "炮友",
  "轮奸",
  "性虎",
  "大sb",
  "肉唇",
  "性虐",
  "色色",
  "浪叫",
  "幼男",
  "屁股",
  "逼奸",
  "和弦",
  "男奴",
  "乳房",
  "色情服务",
  "淫色",
  "色b",
  "潮吹",
  "成人文",
  "原味内衣",
  "屁眼",
  "小逼",
  "淫妇",
  "迷昏药",
  "色界",
  "插你",
  "巨奶",
  "淫騷妹",
  "松岛枫",
  "品香堂",
  "要射了",
  "屄",
  "成人小说",
  "浪妇",
  "轮奸",
  "手淫",
  "hardcore",
  "浑圆",
  "incest",
  "淫穴",
  "口活",
  "精液",
  "流淫",
  "我就色",
  "性伴侣",
  "迷魂药",
  "成人图",
  "口活",
  "嫩逼",
  "操黑",
  "我操",
  "包二奶",
  "伦理片",
  "大波",
  "操逼",
  "淫乱",
  "换妻俱乐部",
  "浪逼",
  "淫虫",
  "暴干",
  "淫女",
  "性奴",
  "兽性",
  "全裸",
  "gay片",
  "菊花洞",
  "淫样",
  "阴阜",
  "妓女",
  "胸推",
  "体奸",
  "发浪",
  "玉乳",
  "淫荡",
  "黄片",
  "美穴",
  "白嫩",
  "大力抽送",
  "淫虐",
  "熟母",
  "汤加丽",
  "婊子",
  "咪咪",
  "偷欢",
  "捏弄",
  "成人小",
  "骚逼",
  "阳具",
  "色欲",
  "骚穴",
  "群交",
  "玉女心经",
  "淫威",
  "胸推",
  "色视频",
  "肉欲",
  "肉缝",
  "人兽",
  "援交",
  "淫电影",
  "乱伦",
  "援助交际",
  "迷幻药",
  "性伙伴",
  "成人游戏",
  "巨屌",
  "兽奸",
  "密穴",
  "嫩穴",
  "阴b",
  "阴阜",
  "爽死我了",
  "淫魔",
  "下体",
  "风月大陆",
  "肛交",
  "伦理电影",
  "阳具",
  "贱比",
  "相奸",
  "操死",
  "买春",
  "fuck",
  "裸露",
  "爽片",
  "偷欢",
  "国产av",
  "要射了",
  "装b",
  "鸡巴",
  "裹本",
  "操烂",
  "母奸",
  "肉棍",
  "肉逼",
  "丝袜",
  "偷拍",
  "我草",
  "白痴",
  "成人色情",
  "巨奶",
  "迷奸",
  "h动漫",
  "干你",
  "色情小说",
  "性奴集中营",
  "蜜液",
  "淫贱",
  "成人网站",
  "插我",
  "招妓",
  "夜勤病栋",
  "拔出来",
  "奸情",
  "情色",
  "虎骑",
  "砲友",
  "淫河",
  "日你妈",
  "兽奸",
  "色盟",
  "淫娃",
  "菊花洞",
  "肏死",
  "黄片",
  "欲仙欲死",
  "就去日",
  "招鸡",
  "内裤",
  "无修正",
  "龟头",
  "色区",
  "欲火",
  "性服务",
  "欲女",
  "欲火",
  "小逼",
  "插进",
  "高潮",
  "狂插",
  "捏弄",
  "sexinsex",
  "骚女",
  "阴精",
  "大乳",
  "淫情",
  "阴毛",
  "男奴",
  "玉穴",
  "日烂",
  "揉乳",
  "成人论坛",
  "狂操",
  "潮喷",
  "应召",
  "暴干",
  "玉蒲团",
  "淫妻",
  "裸陪",
  "一夜情",
  "后庭",
  "裤袜",
  "卧艹",
  "骚比",
  "乳头",
  "肉洞",
  "丝诱",
  "食精",
  "脱内裤",
  "娘西皮",
  "陰道",
  "乱伦类",
  "淫情女",
  "金鳞岂是池中物",
  "干穴",
  "zhengfu",
  "田凤岐",
  "刘云山",
  "政腐",
  "则民",
  "症腐",
  "5毛党",
  "江梳头",
  "石肖",
  "中共封锁",
  "你办事我放心",
  "一中一台",
  "阿曼哈吉",
  "强制拆除",
  "傅锐",
  "高勤荣",
  "shangfang",
  "刘志军",
  "边少斌",
  "霸餐",
  "罢教",
  "出现暴动",
  "华国",
  "违纪",
  "李启红",
  "黄瑶",
  "马恺",
  "丁鑫发",
  "江氏政治委员",
  "江书记",
  "平义杰",
  "死刑缓期",
  "维基百科",
  "中共任用",
  "激流中国",
  "陈水文",
  "政f",
  "黄丽满",
  "郭伯雄",
  "油行",
  "共产专制",
  "警匪",
  "独夫民贼",
  "公产党",
  "许宗衡",
  "援藏",
  "陆肆",
  "台湾应该独立",
  "潘广田山东",
  "供铲谠",
  "江派和胡派",
  "郑筱萸",
  "台湾独立",
  "张高丽",
  "民主女神",
  "周永康",
  "刘志华",
  "温jb",
  "共惨",
  "狗产蛋",
  "李纪周",
  "特共",
  "穆斯林",
  "麻醉弹",
  "王钟麓",
  "撤职。",
  "学百度潮",
  "网评员",
  "薄熙来",
  "焚烧中国国旗",
  "异议人士",
  "黄敬",
  "裆中央",
  "大陆官方",
  "小泽一郎",
  "宪政",
  "当局严密封锁",
  "步qiang",
  "投公",
  "国峰",
  "挡中央",
  "共产党专制",
  "徐鹏航",
  "08xz",
  "罢饭",
  "罢餐",
  "六和谐四",
  "上海帮",
  "爆zha",
  "东突解放组织",
  "孙善武河南",
  "南京军区",
  "刘广智 空军",
  "社会主义灭亡",
  "马英九",
  "台海统一",
  "江派",
  "周小川",
  "锦涛",
  "贡挡",
  "共c党",
  "胡锦涛",
  "江泽民",
  "没收个人财产",
  "抵制罗干",
  "梁湘",
  "炸dan",
  "档中央",
  "低制",
  "民一主",
  "刘瑞龙",
  "杨汇泉",
  "打倒中国",
  "万润南",
  "李堂堂",
  "政府",
  "王华元",
  "于幼军",
  "去中央",
  "苏贞昌",
  "锋同志",
  "王厚宏海南",
  "台百度湾",
  "基地组织",
  "李鹏",
  "中共黑",
  "张秋阳",
  "丹增嘉措",
  "陈德铭",
  "万人骚动",
  "江派人马",
  "耀邦",
  "印尼事件",
  "华主席",
  "封从德",
  "工产党",
  "中华帝国",
  "柳斌杰",
  "抗议",
  "台海大战",
  "周生贤",
  "世界日报",
  "汉维",
  "粟戎生",
  "共产党的末日",
  "供铲党",
  "江贼",
  "入联",
  "焦国标",
  "贡挡",
  "宋平顺",
  "双规",
  "中共邪教",
  "徐绍史",
  "吴定富",
  "袁纯清",
  "猫泽东",
  "张定发",
  "网特",
  "静zuo",
  "温家宝",
  "左翼联盟",
  "中共保命",
  "徇私舞弊",
  "中共帮凶",
  "王振华",
  "中共帝国",
  "王冶坪",
  "阿共",
  "gong和",
  "丘广钟",
  "陈水扁",
  "死刑",
  "duli",
  "托乎提沙比尔",
  "一党专制",
  "弓单",
  "台湾有权独立",
  "江家帮",
  "公产党",
  "毛zd",
  "挡中央",
  "孙瑜",
  "中国人权",
  "皮箱炸弹",
  "中共邪毒素",
  "贪污",
  "巨额财产",
  "塔利班",
  "王炳章",
  "网络评论员",
  "共残裆",
  "江贼民",
  "讨说法",
  "twdl",
  "台海问题",
  "王丹",
  "毛相",
  "张志国",
  "北京当局",
  "张志新",
  "王千源",
  "张辛泰",
  "王宝森",
  "洪清源",
  "胡惊涛",
  "华建敏",
  "湾台",
  "打砸抢",
  "反party",
  "登陆台湾",
  "黄华华",
  "中共退党",
  "游xing",
  "无官正",
  "台湾问题",
  "高严 国家电力",
  "打倒李鹏",
  "x藏",
  "zedong",
  "伊力哈木",
  "张德江",
  "藏毒",
  "颜色革命",
  "贺卫方",
  "六四",
  "倪献策",
  "蔡赴朝",
  "communistparty",
  "中共的罪恶",
  "孟建柱",
  "黄松有",
  "皮黔生",
  "胡j涛",
  "李天羽",
  "王益",
  "江宰民",
  "胡王八",
  "林左鸣",
  "毛太祖",
  "hujintao",
  "丁一平",
  "胡景涛",
  "王超华",
  "习明泽",
  "伊斯兰",
  "霸课",
  "xuechao",
  "辛灏年",
  "麻醉槍",
  "血书",
  "龙新民",
  "zhengfu",
  "徐炳松",
  "ze东",
  "平小邓",
  "东突厥斯坦解放组织",
  "六河蟹四",
  "罢学",
  "希藏",
  "江系人",
  "玩忽职守",
  "麻将透",
  "戒yan",
  "吴仪",
  "四风",
  "中珙",
  "清真",
  "江独裁",
  "陈维席安徽",
  "刘连昆",
  "专制政权",
  "jieyan",
  "江沢民",
  "吕祖善",
  "抵制朱镕基",
  "美国广播公司",
  "清zhen",
  "陈建国",
  "解放tw",
  "静坐",
  "贾庆林",
  "北京政权",
  "抵制共产党",
  "陆四",
  "太子党",
  "64惨案",
  "历史的伤口",
  "网络封锁",
  "主权",
  "请愿",
  "江x",
  "朱镕鸡",
  "戒严",
  "温云松",
  "中华联邦",
  "g匪",
  "八九年",
  "东森新闻网",
  "民进党",
  "西葬",
  "共残主义",
  "李干成",
  "抵制中共",
  "7大军区",
  "共产党腐败",
  "供铲党",
  "曹刚川",
  "政俯",
  "铁英 北京",
  "河蟹社会",
  "官逼民反",
  "李铁映",
  "共产主义的幽灵",
  "信访",
  "封锁消息",
  "蒋彦永",
  "中央领导",
  "人肉炸弹",
  "炸yao",
  "刘长贵",
  "胡长清",
  "刘金宝",
  "兵力部署",
  "昝爱宗",
  "孙晋美",
  "民竹",
  "藏獨",
  "起诉",
  "zhayao",
  "王怀忠",
  "共贪党",
  "党的喉舌",
  "支持台湾",
  "流血冲突",
  "dl喇嘛",
  "温x",
  "常征贵州",
  "周建南",
  "张恩照",
  "藏西",
  "剥夺政治权利",
  "李恩潮",
  "人quan",
  "辛业江 海南",
  "柴玲",
  "张荣坤",
  "恶党",
  "胡紧掏",
  "贪腐财富",
  "陈少勇",
  "规模冲突",
  "共残党",
  "供铲谠",
  "王雪冰",
  "2o年",
  "陈至立",
  "吾尔",
  "抵制温家宝",
  "罢课",
  "政治风波",
  "xi藏",
  "刘奇葆",
  "民猪",
  "汪东兴",
  "zang人",
  "罢食",
  "徐才厚",
  "刘志军",
  "江绵恒",
  "强制捐款",
  "胡紧套",
  "吾尔开希",
  "许运鸿",
  "硒藏",
  "高丽棒子",
  "王沪宁",
  "杜德印",
  "hjt",
  "17da",
  "黄兴国",
  "中国当局",
  "08宪",
  "刘宾雁",
  "国锋",
  "老共",
  "邵松高",
  "李春城",
  "抵zhi",
  "董建华",
  "江太上",
  "蒋捷连",
  "东森电视",
  "令计划",
  "中国zf",
  "共贪党",
  "毒豺",
  "共产专制",
  "高自联",
  "满狗",
  "段义和",
  "三硝基甲苯",
  "六百度四",
  "中华局域网",
  "曾庆红",
  "雪山狮子旗",
  "改革历程",
  "鲁家善 中国交通",
  "康日新",
  "江蛤蟆",
  "邓xp",
  "俞正声",
  "阿拉伯",
  "baodong",
  "陈随便",
  "陈良宇",
  "臧独",
  "政zhi",
  "胡jintao",
  "共匪",
  "麻果丸",
  "成都军区",
  "特贡",
  "记者无疆界",
  "刘维明",
  "严重违纪",
  "宪法法院",
  "请命",
  "g匪",
  "穆罕默德",
  "64运动",
  "许其亮",
  "江核心",
  "受贿罪",
  "共c党",
  "台完",
  "64时期",
  "bbc中文网",
  "收受股票",
  "gongchandang",
  "麻醉药",
  "江系人马",
  "工产党",
  "孙家正",
  "曾荫权",
  "抵制共产主义",
  "吴邦国",
  "默罕默德",
  "王守业",
  "习仲勋",
  "王洛林",
  "亡党亡国",
  "韦泽芳 海南",
  "赖达",
  "官商勾结",
  "回民",
  "回良玉",
  "朱小华光大",
  "殃视",
  "蔡武",
  "中共封网",
  "中共权力斗争",
  "沈跃跃",
  "廖伯年 北京",
  "王维林",
  "铁凝",
  "粉碎四人帮",
  "猫则东",
  "罢考",
  "贺国强",
  "薄熙",
  "杀毙",
  "刘克田",
  "境外媒体",
  "x民党",
  "藏青会",
  "入耳关",
  "天灭中共",
  "youxing",
  "江浙民",
  "you行",
  "马飚",
  "广州军区",
  "gong党",
  "开除党籍",
  "林孔兴",
  "土共",
  "支持zd",
  "韩国狗",
  "中共解体",
  "邱晓华",
  "一党专政",
  "江三条腿",
  "栗智",
  "仇共",
  "麦崇楷",
  "江胡内斗",
  "叛逃美国",
  "灭亡中国",
  "最淫官员",
  "罢参",
  "何洪达",
  "邓质方",
  "李小鹏",
  "李德生",
  "滥用职权",
  "兰州军区",
  "米凤君吉林",
  "供产",
  "徐明",
  "江丑闻",
  "罗云光",
  "泽d",
  "胡春华",
  "youtube",
  "郑光迪",
  "落马",
  "三股势力",
  "韩桂芝",
  "老习",
  "穆罕穆德",
  "江泉集团",
  "台wan",
  "台毒",
  "土共",
  "司马璐",
  "火药配方",
  "中共的血旗",
  "共产王朝",
  "王太华",
  "刘延东",
  "攻占台湾",
  "李月月鸟",
  "共铲",
  "共狗",
  "gong和",
  "联名上书",
  "西独",
  "陈希同",
  "阎明复",
  "麻醉藥",
  "正府",
  "列确",
  "boycott",
  "奚国华",
  "hmtd",
  "学潮",
  "四事件",
  "胡jt",
  "圣战组织",
  "弹药配方",
  "拱铲",
  "由喜贵",
  "安拉",
  "丁子霖",
  "民珠",
  "贰拾周年",
  "张立昌",
  "两岸战争",
  "藏暴乱",
  "国际特赦",
  "gc党",
  "政付",
  "单平",
  "屠光绍",
  "bao炸",
  "江黑心",
  "王岐山",
  "打台湾",
  "赵洪祝",
  "择民",
  "李嘉廷",
  "自杀",
  "罢ke",
  "江氏家族",
  "陈s扁",
  "查克明 华能",
  "剥夺政治权利终身",
  "北京政权",
  "组织集体",
  "姚依林",
  "王式惠重庆",
  "王胜俊",
  "狗产蛋",
  "天按门",
  "经济犯罪",
  "共x党",
  "中共恐惧",
  "baoluan",
  "黄纪诚北京",
  "198964",
  "tibet",
  "侯德健",
  "江胡",
  "xiao平",
  "王鸿举",
  "李小琳",
  "dalai",
  "李长春",
  "正府",
  "无期徒刑",
  "臧人",
  "贪20亿",
  "劣等民族",
  "罗箭",
  "韩福才 青海",
  "抿主",
  "藏独立",
  "拉sa",
  "朱镕基",
  "群体性事件",
  "中央zf",
  "朱容鸡",
  "胡总",
  "付晓光",
  "徐发 黑龙江",
  "肖怀枢",
  "严家其",
  "西z",
  "血洗京城",
  "核蛋",
  "葬独",
  "杨洁篪",
  "人体炸弹",
  "青天白日",
  "九学",
  "湖紧掏",
  "5月35",
  "江哥",
  "共一产一党",
  "英国金融时报",
  "张家盟浙江",
  "xizang",
  "共残主义",
  "吾尔开西",
  "林炎志",
  "台海局势",
  "taiwan",
  "大陆官方",
  "万人大签名",
  "清华帮",
  "gc党",
  "四二六社论",
  "华盛顿邮报",
  "陈同海",
  "江泽慧",
  "刘少奇",
  "热比娅",
  "政俯",
  "麻醉枪",
  "王乐毅",
  "江戏子",
  "江氏集团",
  "藏du",
  "林文漪",
  "路甬祥",
  "程维高",
  "以权谋私",
  "汉人",
  "判处死刑",
  "共残裆",
  "北京军区",
  "梁光烈",
  "贪污腐败",
  "啦萨",
  "马烈孙宁夏",
  "啦撒",
  "姬胜德 总参",
  "jing坐",
  "wikipedia",
  "王乐泉",
  "朱志刚",
  "供产",
  "藏m",
  "三个呆婊",
  "zha药",
  "藏独",
  "无界浏览",
  "a扁",
  "民运人士",
  "北高联",
  "丛福奎",
  "8341部队",
  "共铲",
  "姜殿武 河北",
  "中共党文化",
  "城管暴力执法",
  "中gong",
  "中央zf",
  "恐怖份子",
  "89年春夏之交",
  "江蛤蟆",
  "陈良宇",
  "徐国健",
  "学chao",
  "中共黑帮",
  "上访",
  "解决台湾",
  "温加饱",
  "法轮功",
  "黄镇",
  "红色贵族",
  "陈同海",
  "打倒中共",
  "供铲裆",
  "李达昌",
  "朱容基",
  "宋勇辽宁",
  "jie严",
  "中共",
  "googleblogger",
  "打倒共产党",
  "藏妇会",
  "收复台湾",
  "新疆骚乱",
  "台海战争",
  "chinesedemocracy",
  "无界网络",
  "五毛党",
  "共产党的报应",
  "江毒",
  "达赖",
  "平近习",
  "国wu院",
  "jzm",
  "徐衍东",
  "买别墅",
  "维权",
  "李peng",
  "中gong",
  "梁春禄广西政协",
  "第21集团军",
  "炸弹配方",
  "中共政治游戏",
  "台湾版假币",
  "邓榕",
  "五毛们",
  "成克杰",
  "木仓",
  "宋晨光江西",
  "西藏",
  "周文吉",
  "xjp",
  "症腐",
  "阿旺晋美",
  "胡boss",
  "共产党",
  "权贵集团",
  "维吾",
  "示wei",
  "喝血社会",
  "g产",
  "zha弹",
  "阿共",
  "中华帝国",
  "土g",
  "蒙古分裂分子",
  "选国家主席",
  "李沛瑶",
  "罢吃",
  "苏树林",
  "藏春阁",
  "自由门",
  "自由时报",
  "南街村",
  "上中央",
  "贾廷安",
  "档中央",
  "高官名单",
  "9学",
  "党章",
  "军事社",
  "陈一咨",
  "李宝金",
  "王庆录广西",
  "光复民国",
  "宪章",
  "哒赖",
  "稀藏",
  "常万全",
  "江祸心",
  "胡x",
  "张培莉",
  "江某某",
  "毛泽东",
  "一党执政",
  "人木又",
  "gcd",
  "叶剑英",
  "郭金龙",
  "中南海",
  "秦昌典 重庆",
  "贺子珍",
  "薄一波",
  "抵制江泽民",
  "国民党",
  "柴王群",
  "新闻封锁",
  "产党共",
  "供铲裆",
  "官僚主义",
  "邓朴方",
  "抵制",
  "孙小虹",
  "刘晓竹",
  "国wu院",
  "min主",
  "恶党",
  "江绵康",
  "别梦成灰",
  "中共腐败",
  "zemin",
  "炸药配方",
  "bjork",
  "纽约时报",
  "lasa",
  "zhadan",
  "老j",
  "z东",
  "李建国",
  "人拳",
  "恐怖分子",
  "田凤山。",
  "zf大楼",
  "藏字石",
  "刘永清",
  "杜世成",
  "政zhi",
  "真主",
  "发生暴动",
  "沈彤",
  "回教",
  "启蒙派",
  "江氏政治局",
  "ze民",
  "李荣融",
  "保钓组织",
  "九风",
  "温加保",
  "曾宪梓",
  "李洪志",
  "民泽江",
  "底制",
  "敏主",
  "辛灝年",
  "猫贼洞",
  "政腐",
  "吴文英 纺织",
  "钓鱼岛",
  "范广举 黑龙江",
  "王武龙江苏",
  "江嫡系",
  "g产",
  "疆独",
  "江贼",
  "胡派",
  "da选",
  "gfw",
  "何清涟",
  "fenlie",
  "党产共",
  "石兆彬",
  "大陆当局",
  "蓝田造假案",
  "中共独裁",
  "维汉",
  "紫阳",
  "欧广源",
  "吴振汉",
  "谭作人",
  "反共",
  "日本万岁",
  "白春礼",
  "李源潮",
  "沈图",
  "慕绥新",
  "8的平方事件",
  "于幼军",
  "中共洗脑",
  "蒋公纪念歌",
  "西臧",
  "孟庆平",
  "mayingjiu",
  "共匪",
  "温宝宝",
  "共产主义的幽灵",
  "王昭耀",
  "中国zf",
  "孙鹤龄",
  "张宗海",
  "土g",
  "吴官正",
  "谁是新中国",
  "七大军区",
  "老江",
  "ba课",
  "罢运",
  "方励之",
  "李岚清",
  "抵制胡锦涛",
  "老共",
  "朱川",
  "打倒共产主义",
  "苏晓康",
  "藏人",
  "毛zx",
  "双开",
  "闹独立",
  "亢议",
  "互联网审查",
  "张凯广东",
  "平",
  "李效时",
  "习太子",
  "朱海仑",
  "西奘",
  "六氟化铀",
  "江人马",
  "李大强 神华",
  "吴胜利",
  "希葬",
  "杜世成",
  "中国时报",
  "坦克压大学生",
  "廖锡龙",
  "di制",
  "罢工",
  "台独",
  "侯伍杰",
  "江戏子",
  "温加宝",
  "江zm",
  "军转",
  "中国",
  "台湾国",
  "欧阳德 广东",
  "进攻台湾",
  "陈忠",
  "自动辞",
  "特供",
  "官匪",
  "畏罪自杀。",
  "中国新民党",
  "baozha",
  "邪党",
  "占领台湾",
  "刘知炳",
  "印尼屠华",
  "游行",
  "陈炳德",
  "大赦国际",
  "引起暴动",
  "打倒江主席",
  "戴秉国",
  "李克强",
  "麻醉狗",
  "共x党",
  "星岛日报",
  "里鹏",
  "打倒江泽民",
  "公开信",
  "反分裂",
  "共惨",
  "于飞 广东",
  "江core",
  "4事件",
  "玩女人",
  "自由亚洲",
  "朱云来",
  "胡主席",
  "拉萨",
  "金德琴 中信",
  "灭共",
  "关闭所有论坛",
  "qingzhen",
  "知道64",
  "政付",
  "中国的陷阱",
  "张春江",
  "茳泽民",
  "门安天",
  "温jia宝",
  "胡温",
  "访民",
  "胡海峰",
  "抵制江主席",
  "爱国者同盟",
  "魏京生",
  "中共裁",
  "威胁论",
  "邪党",
  "截访",
  "贰拾年",
  "打倒温家宝",
  "曾培炎",
  "产党共",
  "高智晟",
  "济南军区",
  "西脏",
  "没收个人全部财产",
  "留党察看",
  "台军",
  "十七大",
  "台du",
  "dizhi",
  "江氏集团",
  "taidu",
  "集合",
  "讨伐中宣部",
  "群体事件",
  "贪财物",
  "金盾工程",
  "北京帮",
  "满洲第三帝国",
  "宋平顺",
  "集会",
  "荆福生",
  "藏民",
  "王东明",
  "温如春",
  "打倒罗干",
  "回族",
  "江浙闽",
  "李先念",
  "沈阳军区",
  "4风",
  "陈绍基",
  "陈绍基",
  "9风",
  "寻找林昭的灵魂",
  "解放军",
  "拱铲",
  "来源不明罪",
  "朱作勇甘肃",
  "主席画像",
  "撤销职务",
  "打倒朱镕",
  "霸工",
  "共残党",
  "la萨",
  "解放台湾",
  "形式主义",
  "中共近期权力斗争",
  "强卫",
  "共产王朝",
  "王兆国",
  "renquan",
  "台弯",
  "胡海清",
  "告全国同胞书",
  "李瑞环",
  "阿扁",
  "zangdu",
  "打倒胡锦涛",
  "十7大",
  "林树森",
  "统一台湾",
  "有期徒刑",
  "20和谐年",
  "江猪",
  "鞑子",
  "王学军",
  "公头",
  "江主席",
  "靖志远",
  "邓晓平",
  "邓小平",
  "抵制李鹏",
  "啦沙",
  "回回",
  "示威",
  "火乍",
  "台海危机",
  "江主席",
  "中珙",
  "共狗",
  "政f",
  "台湾共和国",
  "gong党",
  "江绵恒",
  "李学举",
  "吕德彬",
  "仇共",
  "张曙",
  "裆中央",
  "习近平",
  "王有杰河南",
  "胡平",
  "北京风波",
  "张国光",
  "汪兆钧",
  "平反",
  "刘方仁",
];

/**
 * 检测文本中的敏感词位置和信息
 * 遍历整个文本，查找所有敏感词的位置、内容和长度
 * @param {string} novel_text - 需要检测的小说文本内容
 * @returns {Array} sensitive_word_loc - 包含敏感词信息的数组，每个元素包含：
 *   - sensitive_word: 敏感词内容
 *   - loc: 敏感词在文本中的起始位置
 *   - sensitive_word_length: 敏感词的字符长度
 */
function detective_sensitive_words(novel_text) {
  // 初始化敏感词位置信息数组
  let sensitive_word_loc = [];

  // 遍历文本的每个字符位置
  for (let loc = 0, len = novel_text.length; loc < len; loc++) {
    // 遍历敏感词库中的每个敏感词
    for (let sensitive_word of sw) {
      let sensitive_word_length = sensitive_word.length;

      // 从当前位置截取与敏感词长度相同的文本片段
      let novel_single_word = novel_text.substring(
        loc,
        loc + sensitive_word_length
      );

      // 检查截取的文本片段是否与敏感词匹配
      if (sensitive_word === novel_single_word) {
        // 记录敏感词的详细信息
        sensitive_word_loc.push({
          sensitive_word: sensitive_word, // 敏感词内容
          loc: loc, // 敏感词位置
          sensitive_word_length: sensitive_word_length, // 敏感词长度
        });
        break; // 找到匹配后跳出内层循环，继续检查下一个位置
      }
    }
  }

  // 输出检测结果
  // console.log(sensitive_word_loc);
  return sensitive_word_loc;
}

/**
 * 将敏感词替换为相同长度的星号符号
 * 根据敏感词的字符长度生成对应数量的星号
 * @param {string} sensitive_word - 需要替换的敏感词
 * @returns {string} 与敏感词长度相同的星号字符串
 */
function replace_sensitive_word_with_symbols(sensitive_word) {
  // 获取敏感词的字符长度
  let word_length = sensitive_word.length;

  // 生成相同长度的星号字符串
  return "*".repeat(word_length);
}

// 使用全局的 pinyinPro
const { pinyin } = pinyinPro;
/**
 * 将敏感词全部转换为拼音
 * @param {string} sensitive_word - 需要转换的敏感词
 * @returns {string} 转换后的拼音
 */
function replace_sensitive_word_with_pinyin_all(sensitive_word) {
  // 基础拼音转换
  const result = pinyin(sensitive_word, { toneType: "none" });
  return result;
}

/**
 * 将敏感词中的一个随机汉字换成拼音
 * @param {string} sensitive_word - 需要转换的敏感词
 * @returns {string} 转换后的包含拼音的汉字
 */
function replace_sensitive_word_with_pinyin_one(sensitive_word) {
  // 随机选择一个汉字
  let random_index = Math.floor(Math.random() * sensitive_word.length);
  let random_char = sensitive_word[random_index];

  // 转换为拼音
  let pinyin_result = pinyin(random_char, { toneType: "none" });

  // 正确提取拼音字符串
  let pinyin_string = pinyin_result; // 获取第一个字符的第一个拼音

  // 替换原始字符串中的随机汉字
  let result = sensitive_word.replace(random_char, pinyin_string);

  return result;
}

/**
 * 根据指定方式替换文本中的敏感词
 * 先检测文本中的所有敏感词，然后根据替换方式进行相应的替换操作
 * @param {string} novel_text - 需要处理的小说文本内容
 * @param {string} replace_way - 替换方式，目前支持 "symbol"（用星号替换）
 * @returns {string} 处理后的文本内容
 */
function replace_sensitive_words(novel_text, replace_way) {
  // 检测文本中的所有敏感词位置和信息
  let sensitive_word_array = detective_sensitive_words(novel_text);

  // 根据替换方式进行处理
  // 使用*进行替换
  if (replace_way === "symbol") {
    // 遍历所有检测到的敏感词
    for (let sensitive_word_obj of sensitive_word_array) {
      let sensitive_word = sensitive_word_obj.sensitive_word;

      // 生成替换用的星号字符串
      let replace_word = replace_sensitive_word_with_symbols(sensitive_word);

      // 在文本中替换敏感词（注意：这里只会替换第一个匹配项）
      novel_text = novel_text.replace(sensitive_word, replace_word);
    }
  }
  // 使用拼音随机替换一个汉字
  else if (replace_way === "pinyin_one") {
    // 遍历所有检测到的敏感词
    for (let sensitive_word_obj of sensitive_word_array) {
      let sensitive_word = sensitive_word_obj.sensitive_word;

      // 生成替换用的拼音字符串
      let replace_word = replace_sensitive_word_with_pinyin_one(sensitive_word);

      // 在文本中替换敏感词（注意：这里只会替换第一个匹配项）
      novel_text = novel_text.replace(sensitive_word, replace_word);
    }
  }
  // 使用拼音全部替换
  else if (replace_way === "pinyin_all") {
    // 遍历所有检测到的敏感词
    for (let sensitive_word_obj of sensitive_word_array) {
      let sensitive_word = sensitive_word_obj.sensitive_word;

      // 生成替换用的拼音字符串
      let replace_word = replace_sensitive_word_with_pinyin_all(sensitive_word);

      // 在文本中替换敏感词（注意：这里只会替换第一个匹配项）
      novel_text = novel_text.replace(sensitive_word, replace_word);
    }
  }

  // 返回处理后的文本
  return novel_text;
}

export { replace_sensitive_words };
